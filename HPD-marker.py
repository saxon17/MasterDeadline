...
def block_Dataframe(pcap_file_path):
    ...
    return df2
def GetDurationFromHttpReq(pcapFilePath):
    import  os
    import  re
    cmd = 'Parse_pcap.exe  '+ pcapFilePath
    # print (os.popen(cmd).read())
    HTTP_Request_Response = os.popen(cmd).readlines()
    # print( HTTP_Request_Response)

    YoutueGetLST =  [req for req in HTTP_Request_Response if re.search(r"dur=\d+", req)]


    m = re.search(r"dur=\d+\.\d+", YoutueGetLST[0])
    if m:
        return m.group(0)[4:]
def main(argv):
    ...
    for f in files:
        ...
        VedioDuration = GetDurationFromHttpReq(f)

        print 'This vedio dur is ' ,VedioDuration,'s'
        ...
        df360P['BitRate'] = df360P['block_size'].sum()*8/float(VedioDuration)
